...
dev = "c_ega"  # ur username

token = "6875762803:AAENJstqwZcd6B-nsOn0NUuKFolajGhpnl4"
...
def welcome(message):
    ...
    x = requests.get(
        f"https://api.telegram.org/bot{token}/getchatmember?chat_id=@{chat_id}&user_id={message.from_user.id}")
    if any(["member" in x.text, "administrator" in x.text, "creator" in x.text]):
        ...
        if str(message.from_user.id) == str(admin):
            ...
            Tho2 = types.InlineKeyboardButton(
                "أرسل التخزين", callback_data="file")
            Tho3 = types.InlineKeyboardButton(
                f"الأحصائيات", callback_data="info")
            Tho4 = types.InlineKeyboardButton(
                f"حظر مستخدم", callback_data="ban")
            Tho5 = types.InlineKeyboardButton(
                f"مسح المحظورين", callback_data="allun")
            Tho6 = types.InlineKeyboardButton(
                f"الغاء حظر", callback_data="unban")
            Tho7 = types.InlineKeyboardButton(
                f"رفع أدمن", callback_data="adad")
            Tho8 = types.InlineKeyboardButton(
                f"تنزيل أدمن", callback_data="unad")
            Tho9 = types.InlineKeyboardButton(
                f"مسح جميع الادمنية", callback_data="rmad")
            ...
            keyboard.row(Tho9)
            bot.reply_to(message, f"*أهلا بك عزيزي المالك*",
                         reply_markup=keyboard)
            btn = types.InlineKeyboardMarkup()
            btn.row_width = 1
            ah = types.InlineKeyboardButton(text="المطور", url=f"t.me/{dev}")
            btn.add(ah)
            bot.send_photo(message.chat.id, 'https://t.me/aahhjdmthoncj/13')
            bot.reply_to(
                message, f"مرحبا بك <strong>{nam}</strong> في بوت المطور حمودي\nيوزرك @{user}\nايديك <code>{id}</code>\nلضهور الاوامر اكتب الاومر", parse_mode='html', reply_markup=btn)
            ...
            bot.reply_to(message, f"*أهلا بك عزيزي الادمن*",
                         reply_markup=keyboard)
            bot.send_photo(message.chat.id, 'https://t.me/aahhjdmthoncj/13')
            bot.reply_to(
                message, f"مرحبا بك <strong>{nam}</strong> في بوت المطور حمودي\nيوزرك @{user}\nايديك <code>{id}</code>\nلضهور الاوامر اكتب الاومر", parse_mode='html')
        ...
        bot.reply_to(
            message, f"*لازم تشترك بقناة البوت حتى تكدر تستخدمة\n\n- @{chat_id}*")
...
def calldata(call):
    ...
    if call.data == "info":
        ...
        bot.send_message(
            call.message.chat.id, f"*أهلا بك عزيزي الادمن في قسم الاحصائيات\nعدد مستخدمين البوت: {qa}.\nعدد المحظورين: {ar}.\nعدد الأدمنية: {ad}.*")
        Th.close()
        Of.close()
        adr.close()
    ...
    if call.data == "allun":
        try:
            ...
            os.remove("ban.txt")
            bot.send_message(
                admin, "*تم مسح جميع المحظورين و أرسال لك نسخة احتياطية*")
        except:
    ...
    if call.data == "unban":
        bot.send_message(call.message.chat.id,
                         "*أرسل الايدي الذي تريد الغاء حظره*")
